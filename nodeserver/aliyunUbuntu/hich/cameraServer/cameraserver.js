...
let isRecord = true;
let isNewDataReceived = false; // 标记是否有新数据
...
function saveImagePeriodically() {
    saveImageInterval = setInterval(() => {
        if (!isNewDataReceived) {
            console.log("No new data received. Skipping save.");
            return;
        }

        isNewDataReceived = false; // 保存后重置标记
        const now = new Date();
        ...
        fs.readFile(VIDEO_STREAM_PATH, (err, data) => {
            ...
            if (!data || data.length <= 100) {
                console.error('Invalid data. Skipping save.');
                return;
            }
